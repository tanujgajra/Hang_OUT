...
function showLevels(l){
  ...
  c.innerHTML="";
  let u=parseInt(localStorage.getItem(l)) || 1;
  for(let i=1;i<=30;i++){
...
function startLevel(l,n){
  ...
  // Random word for every level
  const levelWords = words[l];
...
function checkGame(){
  if(!displayWord.includes("_")){
    ...
    // Unlock next level
    let u=parseInt(localStorage.getItem(currentDifficulty)) || 1;
...
/* WELCOME PAGE ANIMATED TEXT */
function animateWelcomeText(){
  const container = document.getElementById("welcomeTextAnimated");
  setInterval(()=>{
    const phrase = welcomePhrases[Math.floor(Math.random() * welcomePhrases.length)];
    container.innerText = phrase;
